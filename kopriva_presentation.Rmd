...
output:
  ioslides_presentation:
    ...
    widescreen: true
    css: logo.css
subtitle: "Allison Theobold, Ph.D. Candidate in Statistics </br> Adviser: Dr. Stacey Hancock"
fontsize: 10pt
---
...
<img src="figures/hamburger.jpg" style="float:right"> <p>

- Research Motivation  

- Climate of Computing -- Environmental Sciences & Statistics  

- Research on Data Science Workshops  
...
But the `R` functions you saw in your courses require for your data to be in long 
format.
...
![](figures/need_for_computing.JPG)
...
- None of these conversations pay attention to the role a students' 
statistics education plays in their attainment of the computing skills 
...
- 2005: `ggplot2` beginning of user friendly `R` tools (Wickham)

- 2012: `RMarkdown` for embedding code in reports
...
- 2014: ASA updated guidelines for undergraduate programs in Statistics 
...
- 2017: `tidyverse` houses all of the packages necessary for a clean data science workflow 
...
<!-- Generally, each function should do one thing well, and you should be able to  -->
<!-- describe the purpose of the function in one sentence -->
...
3. What are the experiences of individuals attending theses tailored data 
science workshops?   
...
| <font size="5"> Research Question | <font size="5">Design Phase | <font size="5">Data Collected |    
|:----------------|:------------:|:----------------:|  
| <font size="5"> What do environmental science faculty members identify as the key computing skills graduate student require to implement statistics in environmental science research?  | <font size="5"> Phase 1  | <font size="5"> Faculty interviews |  
| <font size="5"> How can these key computing skills be incorporated into currently existing workshop materials? | <font size="5"> Phase 2  | <font size="5"> Carpentries curriculum materials |  
...
|<font size="5"> Research Question | <font size="5"> Design Phase | <font size="5"> Data Collected |      
|------------------|:--------------:|:----------------:|  
| <font size="5"> What are the experiences of individuals attending data science workshops? | <font size="5"> Phase 3 |  <font size="5"> Pre- and post-workshop surveys|   
| <font size="5"> Does the content of these workshops reflect the data science skills used by graduate students in their research? |  | <font size="5"> Collection of graduate students' research (`R`) code |  
...
- "I think that more and more in our field my generation is sort of just catching up the next generation."  

- "Increasingly faculty feel that way like they're they're not at the forefront of the programming abilities and so their students are kind of being self-taught and then are often ahead of them."  

- "I feel personally out of touch, because [students] work in `R` and I haven't taken the time to learn `R`, because of my training and my age."
...
<!-- With 5 available lessons -- written by Roger Peng and collaborators -->
...
## Benefits of Data Science Workshops

- Approachable training resources for researchers hoping to develop 
computational skills are scarce (Teal et al., 2015).   
...
- Content: skills distilled from interviews infused into Data & Software Carpentry `R` lessons. 
    
- Structure: interactive exercises build confidence 
    
- Context: environmental science data promote learning transfer

## `R` for Data Science Workshops    

| <font size="5"> |                              |    
|:----------------|:----------------------------:|  
| <font size="5"> Introduction to `R` | <font size="5"> objects in `R`, working in `R`, inspecting, extracting, cleaning, summarizing, and visualizing data with base `R` tools |  
| <font size="5"> Intermediate `R` | <font size="5"> relational and logical operators, conditional statements, looping, and user-defined functions  |  
| <font size="5"> Data Wrangling in `R` |  <font size="5"> data wrangling with `dplyr`, `stringr`, `forcats`, and `lubridate`, data organization with `tidyr` |  
| <font size="5"> Data Visualization in `R` | <font size="5"> grammar of graphics with `ggplot2` |

## Workshop Tools 

- RStudio 
    * Access to the resources of `R`, in a comfortable working environment 
...
- RMarkdown Documents  
    * Promotes organized code and clean workspaces   
...
<!-- Each piece should be easily understood in isolation, and have a standard way to combine with other pieces. -->
...
```{r, echo = FALSE, warning = FALSE, fig.width = 20, fig.height = 10, fig.align = 'center'}
wl <- fish %>%
        ggplot(aes(x = length, y = weight, color = species)) + 
        geom_point()
len <- fish %>% 
...
```{r, out.width = "900px", out.height = "475px", echo = FALSE}
...
languages <- all_ws %>% 
  ...
  mutate(language = case_when(language == "other_language" ~ "R", 
                              language == "java" ~ "JavaScript",
                              language == "python" ~ "Python",
                              language == "no_languages" ~ "None",
                              language == "fortran" ~ "Fortran",
                              TRUE ~ language ), 
         Language = language) %>% 
  filter(familiar == "Yes" | familiar == "Unfamiliar" | familiar == "None") %>% 
  mutate(Language = factor(Language, 
                           levels = c("None", "R", "Python", "JavaScript", 
                                      "SQL", "C", "Fortran")
                           )
  ) %>% 
  group_by(Language) %>%
  ...
  ggplot(mapping = aes(x = Language, y = Freq, fill = Language)) + 
  coord_flip() + 
  geom_bar(stat = "identity") + 
  ylim(0, 80) +
  #geom_text(aes(label = Freq, y = Freq + 1), vjust = 0, size = 10) +
...
stat_courses <- data.frame(Course = c("Introductory Statistics", 
                                      "Applied Statistics", 
                                      "None", 
                                      "Discipline Specific Intro Stat",
                                      "Intermediate Statistics", 
                                      "Experimental Design", 
                                      "Probability Theory"), 
                           Freq = c(46, 42, 24, 20, 10, 8, 6))
                                      
stat <- stat_courses %>% 
  mutate(Course = factor(Course, 
                         levels = c("Introductory Statistics", 
                                      "Applied Statistics", 
                                      "None", 
                                      "Discipline Specific Intro Stat",
                                      "Intermediate Statistics", 
  ...
  ggplot(mapping = aes(x = Course, y = Freq, fill = Course)) + 
  coord_flip() + 
  geom_bar(stat = "identity") + 
  ylim(0, 50) +
  #geom_text(aes(label = Freq, y = Freq + 1), vjust = 0, size = 10) +
...
grid.arrange(languages, stat, nrow = 1, ncol = 2, widths = c(4, 5))
```
...
all_ws %>% 
  ...
  mutate(Reason = reason, 
         Reason = case_when(Reason == "research_help" ~ "Research Help", 
                            Reason == "coursework_help" ~ "Coursework Help", 
                            Reason == "flier" ~ "Flier", 
                            Reason == "refresh_update" ~ "Refresh/Update Skills", 
                            Reason == "grad_or_career_prep" ~ "Grad/Career Prep", 
                            Reason == "dept_recommend" ~ "Dept Recommend", 
                            Reason == "adviser_recommend" ~ "Adviser Recommend", 
                            Reason == "professional_develop" ~ "Professional Development"), 
         Reason = factor(Reason, 
                         levels = c("Research Help", 
                                    "Coursework Help", 
                                    "Flier", 
...
ggplot(mapping = aes(x = Reason, y = n, fill = Reason)) + 
  coord_flip() + 
  geom_bar(stat = "identity") + 
  ylim(0, 60) +
  #geom_text(aes(label = Freq, y = Freq + 1), vjust = 0, size = 10) +
...
all_ws %>% 
  ...
  mutate(Resource = resource, 
         Resource = case_when(Resource == "internet_resources" ~ "Internet", 
                              Resource == "course_resources" ~ "Courses", 
                              Resource == "peer_resources" ~ "Peers", 
                              Resource == "lab_resources" ~ "Lab Mates", 
                              Resource == "adviser_resource" ~ "Adviser", 
                              Resource == "no_resources" ~ "None", 
                              Resource == "previous_workshops" ~ "These Workshops"), 
...
ggplot(mapping = aes(x = Resource, y = n, fill = Resource)) + 
  coord_flip() + 
  geom_bar(stat = "identity") + 
  ylim(0, 60) +
  #geom_text(aes(label = Freq, y = Freq + 1), vjust = 0, size = 10) +
...
- Conditional statements
    * Creating new variables by mutating existing variables
    * Function breaks 
...
- Attendance of these workshops by many individuals outside of environmental 
science disciplines brings to question if this type of tailored design work is 
necessary. 
...
<img src="figures/Rpubs.JPG" style="float:left"> 

</br>
</br>

http://bit.ly/kopriva_theobold

</br>
</br>

<img src="figures/missouri.jpg" style="float:left"> <p>
...
https://github.com/atheobold/data-science-workshops

<img src="figures/fish.jpg" style="float:right"> <p>

</br> 

Workshops in RStudio Cloud:

http://bit.ly/introductionR  
http://bit.ly/intermediate-R  
http://bit.ly/data-vizz  
http://bit.ly/data-wrangle  

</br>
allisontheobld@montana.edu  
