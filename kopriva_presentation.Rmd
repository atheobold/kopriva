...
## Workshop Attendance by Department Affiliation 
...
all_ws <- read_csv("data/all_ws_clean.csv") %>% 
  ...
  filter(program != "Postdoc/Faculty/Staff") 
## removing these departments eliminates 10 individuals

dept_levs <- c("LRES", "Ecology", "Plant Sciences", "Biochem/Microbio",  
               "Engineering", "Math/Stats", "Animal Sciences", 
               "Business", "Earth Sciences", "Biology",
               "Agriculture", "CS", "Education", "Health Sciences", 
               "Poly Sci", "Psych")
  
degree_levs <- c("Bachelors","Master's", "Doctorate", "Faculty", "Staff", "Other")
...
attendance <- all_ws %>%
  ...
  mutate(Department = case_when(grepl("Land Resources", Department) ~ 
                                ...
                                TRUE ~ Department), 
         Degree = case_when(Degree == "Bachelors" ~ "Bachelor's", 
                            TRUE ~ Degree)) %>%
  mutate(Department = factor(Department, levels = dept_levs), 
         Degree = factor(Degree, levels = degree_levs)) %>% 
...
## Resouces Used to Learn `R` 


## Do These Skills Capture What Researchers Use?

## Future Directions -- Learning Trajectory for Computing with Data  {data-background=#C1C6C8}
